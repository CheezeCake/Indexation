...
public class AndOrQueryEngine
{
	...
	public AndOrQueryEngine(Index index)
	{
		this.index = index;
	}

	public void splitAndQuery(String query, List<List<Posting>> result)
	{
	...
	void splitOrQuery(String query, List<List<List<Posting>>> result)
	{
		String[] expressions = query.split(",");

		for (String expression : expressions) {
			List<List<Posting>> andResult = new LinkedList<>();
			splitAndQuery(expression, andResult);
	...
	private List<Posting> processDisjunctions(List<List<Posting>> postings)
	{
		...
		while (it.hasNext())
			ret = processDisjunction(ret, it.next());
	...
	public List<Posting> processQuery(String query)
	{
		...
		List<List<List<Posting>>> orPostings = new LinkedList<>();
		splitOrQuery(query, orPostings);

		List<List<Posting>> andResults = new LinkedList<>();
		for (List<List<Posting>> andPostings : orPostings)
			andResults.add(processConjunctions(andPostings));

		List<Posting> ret = processDisjunctions(andResults);
